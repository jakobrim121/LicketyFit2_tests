...
#                     continue
                ...
                if s<-1000:
                    ...
                    scale_sub, s_b_sub = find_scale_for_pmts(
...
#                 def power_law(x,B,n_fit):
#                     max_ = 1/(1+B)
#                     norm = ((x**n_fit) / (1 + B * x**n_fit))/max_
#                     return norm
...
#                     print('')
                ...
                #pwr_corr = power_law(cost, B, n_fit)
                ...
                mean_pe = self.intensity * corr * frac_area * wl_fac
                #print(mean_pe)
...
#         scale[s_eff is None] = 0.
        ...
        scale[s_eff < pmt_radius] = (s_eff[s_eff < pmt_radius] + pmt_radius) / (2.*pmt_radius)
        valid_s = s_eff >= -pmt_radius
        scale = scale * valid_s
        s_eff = np.where(valid_s, s_eff, 0.0)
        #print('LENGTH 2',len(s_eff))
        ...
        r = np.linalg.norm(to_pmt, axis=1) + 1.0  # +1 mm safety
...
#         def wl_corr(x, ymin, ymax, x50, n):
#             x = np.asarray(x)
#             # avoid divide-by-zero if you ever have x=0
#             x_safe = np.maximum(x, 1e-12)
#             return ymin + (ymax - ymin) / (1.0 + (x50 / x_safe)**n)
...
#         n_wl    = 2.1020
        ...
        # ================================================================
        mean_pes = self.intensity * corr * scale * wl_fac
